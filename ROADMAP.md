...
## ✅ Recently Completed
- [x] **Batch URL Resolution (subsequent batches)**: Fixed bug where only the first batch of 10 links in a folder was being resolved. Root cause: `seenNormalizedUrls` was initialized with all original (pre-resolution) URLs and never removed them when a URL resolved to a different target. When a bookmark in a later batch had an original URL matching the resolved target of an earlier bookmark (e.g., `bit.ly/abc` → `example.com/article`, and later bookmark was already `example.com/article`), it was falsely detected as a duplicate and removed. Fix: track URL transitions with a `normalizedToOriginals` map, remove the original URL from the seen-set when a bookmark resolves to a new URL, and only flag duplicates when the resolved URL belongs to a different original bookmark.
