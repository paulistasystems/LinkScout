...
## ✅ Recently Completed
- [x] **Sidebar load performance regression fix (v2.7.51)**: Fixed the sidebar taking too long to load. Root cause: v2.7.47 changed `loadBookmarks()` to run three heavy operations serially on every open — `syncBookmarks` (full sync: dedup + reverse-sync purge), `restoreFromDB` (emergency restore), then `getBookmarkTree` — each independently doing a full recursive bookmark traversal plus a full IndexedDB `getAll()` (≈3 tree walks + 2 DB reads per open). Both heavy operations already run once at startup. Fixes: (1) the hot path now uses `syncBookmarksLightweight` (adds only missing bookmarks) + `getBookmarkTree`; (2) `restoreFromDB` is only invoked when the returned tree is empty, preserving the v2.7.46 two-way-sync safety net without paying for it on every open; (3) `buildTree` in `getBookmarkTreeForSidebar` now recurses into subfolders in parallel via `Promise.all` instead of awaiting each `getChildren` sequentially.
